...
namespace MilkMatrix.Milk.Implementations.Route.RouteContractor
{
    public class RouteContractorService : IRouteContractorService
    {
        ...
        public async Task<RouteContractorResponse> InsertRouteContractor(RouteContractorInsertRequest request)
        {
            try
            {
                ...
                var result = await repository.AddAsync(RouteContractorQueries.AddRouteContractor, requestParams, CommandType.StoredProcedure);

                if (result.StartsWith("Error"))
                    throw new Exception($"Stored Procedure Error: {result}");

                if (int.TryParse(result, out int newRouteContractorId))
                    return await GetRouteContractorById(newRouteContractorId);

                throw new Exception($"Failed to retrieve the new RouteContractorId. SP Result: {result}");
            }
            catch (Exception ex)
            {
