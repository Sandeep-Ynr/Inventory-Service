...
public interface IQueryMultipleData
{
    ...
    Task<(IEnumerable<T1> t1, IEnumerable<T2> t2, IEnumerable<T3> t3, IEnumerable<T4> t4, IEnumerable<T5> t5, IEnumerable<T6> t6)> GetMultiDetailsAsync<T1, T2, T3, T4, T5, T6>(string query, string connKey, Dictionary<string, object>? parameters, int? commandTimeOut, CommandType commandType = CommandType.StoredProcedure);
}
